...
if($limit=="pull")
{
	$wherex = "and postid > $start";
	$limit = 10;
	$start = 0;
}

$urut = "order by postid desc";
...
$sql = "select count(*) as jml from tbl_post where isi!='' $wherex";
$hsl = sql( $sql);
...
$perintah	= "select postid,isi,username,userid,userfullname,touserid,tousername,touserfullname,tanggal,jmlLike,groupnya,type,media,via,jmlkomen from tbl_post where isi!='' $wherex $urut  limit $start, $limit";
$hasil = sql( $perintah);
...
while ($data =  sql_fetch_data($hasil))
 {	
		...
		else
		{
			...
			$u++;
			unset($mjenis,$murl,$mcontent,$hapusposting);
