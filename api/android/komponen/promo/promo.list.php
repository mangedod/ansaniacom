...
$sql = "select count(*) as jml from tbl_promo where published='1' and gambar!='' $where";
$hsl = sql( $sql);
...
$mysql = "select id,ringkas,nama,create_date,alias,gambar,gambar1,views from tbl_promo where published='1' and gambar!='' $where order by id desc limit $start, $limit";
$hasil = sql( $mysql);
...
while ($data =  sql_fetch_data($hasil)) {	
	...
	if(!empty($gambar)) $gambar = "$fulldomain/gambar/promo/$gambar";
	 else $gambar = "";
