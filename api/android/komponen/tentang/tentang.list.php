...
$sql = "select count(*) as jml from tbl_tentangseft where published='1' $where";
$hsl = sql( $sql);
...
$mysql = "select id,ringkas,nama,create_date,alias,gambar,gambar1,views from tbl_tentangseft where published='1' $where order by id asc limit $start, $limit";
$hasil = sql( $mysql);
...
while ($data =  sql_fetch_data($hasil)) {	
	...
	if(!empty($gambar)) $gambar = "$fulldomain/gambar/tentangseft/$gambar";
	 else $gambar = "";
