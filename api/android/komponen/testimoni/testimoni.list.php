...
$sql = "select count(*) as jml from tbl_testimoni where published='1' and gambar!='' $where";
$hsl = sql( $sql);
...
$mysql = "select id,nama,testimoni,company,create_date,gambar from tbl_testimoni where published='1' and gambar!='' $where order by id desc limit $start, $limit";
$hasil = sql( $mysql);
...
while ($data =  sql_fetch_data($hasil)) {	
	...
	$datadetail[] = array("id"=>$id,"no"=>$i,"nama"=>$nama,"ringkas"=>$ringkas,"tanggal"=>$tanggal1,"date"=>$tanggal,"company"=>$company,"url"=>$link,"gambar"=>$gambar,"views"=>$views);
	$i++;
