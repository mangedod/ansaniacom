...
$i = 1;
$a = 1;
$sql = "select transaksidetailid, produkpostid, jumlah,berat from tbl_transaksi_detail where transaksiid='$transaksiid' order by transaksidetailid desc ";
$hsl = sql($sql);
...
{		
	...
	else
	{
		...
		header("location: $fulldomain"."/member/cart");
	}
}
else
{
	$salah = "Total quantity of products purchased insufficient. Please reduce the quantity of your product to continue shopping.<br><br>\n";
	$tpl->assign("style","alert-danger");
	$tpl->assign("salah",$salah);
}

$tpl->assign("namarubrik","Checkout transactions");
