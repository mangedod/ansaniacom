<?php
	$kodeinvoice = $var[4];
		...
		{
			$sql1 	= "select transaksiid,invoiceid,totaltagihan,totaltagihanafterdiskon,totalinvoice,ongkoskirim, bank_tujuan from tbl_transaksi where invoiceid='$invoiceid'";
			$hsl1 	= sql($sql1);
		...
		//BANK PENGIRIM
		$sql8        = "select bankid,namabank from tbl_bank order by namabank asc";
		$res8        = sql($sql8);
		while ($row8 = sql_fetch_data($res8)) 
		{
			$id			= $row8['bankid'];
			$namabank	= $row8['namabank'];
			
			$bankpengirim[$id] = array("id"=>$id, "nama"=>$namabank, "id_bank"=>$id_bank);
		}
		sql_free_result($res8);
		$tpl->assign("bankpengirim",$bankpengirim);
