<?php
	...
	{*/
		...
		$datetransfer            = tanggal_english($row1['batastransfer']);
		...
		while ($row = sql_fetch_data($hsl))
		{
			...
			$dtprodukpos = sql_get_var_row("select title,nomor, kodeproduk, misc_harga, misc_diskon, warnaid, sizeid from tbl_product_post where produkpostid='$produkpostid'");
			$namaprod    = $dtprodukpos['title'];
			$nomor    = $dtprodukpos['nomor'];
			...
			$dt_keranjang[$transaksidetailid] = array("transaksidetailid"=>$transaksidetailid,"xx"=>$xx,"nama"=>$namaprod,"nomor"=>$nomor,"harga"=>$harga,"totalharga"=>$total,"qty"=>$jumlah,"kodeproduk"=>$kodeproduk,"harga_asli"=>$harga_asli,"hargadiskon"=>$hargadiskon,"namawarna"=>$namawarna,"size"=>$size);
			$xx++;
