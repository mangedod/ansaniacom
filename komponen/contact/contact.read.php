<?php 
	...
	if($_POST['action'] == "savepesan")
	{
		...
		{
			$firstname = cleaninsert($_POST['firstname']);
			$lastname  = cleaninsert($_POST['lastname']);
			$nama      = "$firstname $lastname";
			$email     = cleaninsert($_POST['useremail']);
			$phone     = cleaninsert($_POST['phone']);
			...
			$perintah	= "insert into tbl_contact (`id`,`nama`,`email`,`pesan`,`phone`,`ip`, `tanggal`) values ('$idbaru','$nama','$email','$pesan','$phone','$ip', '$tanggal')";
			$hasil		= sql($perintah);
