 <?php 
		...
		$datagaleri[$catId] = array("id"=>$id,"nama"=>$nama,"ringkas"=>$ringkas,"gambar"=>"$fulldomain/gambar/$gambar","url"=>"$domain$cleanurl/$kanal/read/$catId/1/$alias");
		...
		if ($hlm > 1){
			$prev = $hlm - 1;
			$stringpage[$pageid] = array("nama"=>"First","link"=>"$fulldomain/$kanal/$aksi/1");
			$pageid++;
			$stringpage[$pageid] = array("nama"=>"Previous","link"=>"$fulldomain/$kanal/$aksi/$prev");
		...
		if ($hlm < $hlm_tot){
			...
			$pageid++;
			$stringpage[$pageid] = array("nama"=>"Last","link"=>"$fulldomain/$kanal/$aksi/$hlm_tot");
		}
		else
		{
