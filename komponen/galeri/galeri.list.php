...
while ($data = sql_fetch_data($hasil))
{
	...
	if(!empty($gambar))
	{
		$datagaleri[$secid] = array("id"=>$id,"nama"=>$nama,"ringkas"=>$ringkas,"tanggal"=>tanggal($create_date),"gambar"=>"$fulldomain/gambar/galeri/$gambar","url"=>"$fulldomain/$kanal/read/$secid/$idgambar/$alias");
	}
...
if ($hlm > 1){
	$prev = $hlm - 1;
	$stringpage[$pageid] = array("nama"=>"Awal","link"=>"$fulldomain/$kanal/list/1");
	$pageid++;
	$stringpage[$pageid] = array("nama"=>"Sebelumnya","link"=>"$fulldomain/$kanal/list/$prev");
...
for ($ii=$hlm3; $ii <= $hlm_tot and $ii<=$hlm4; $ii++){
	if ($ii==$hlm){
		...
		$stringpage[$pageid] = array("nama"=>"$ii","link"=>"$fulldomain/$kanal/list/$ii");
	}
	$pageid++;
}
if ($hlm < $hlm_tot){
	$next = $hlm + 1;
	$stringpage[$pageid] = array("nama"=>"Selanjutnya","link"=>"$fulldomain/$kanal/list/$next");
	$pageid++;
	$stringpage[$pageid] = array("nama"=>"Akhir","link"=>"$fulldomain/$kanal/list/$hlm_tot");
}
else
{
