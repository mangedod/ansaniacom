...
while ($row = sql_fetch_data($hsl1))
{
	...
	while ($row = sql_fetch_data($hsl))
	{
		...
		$datagaleridetail[$id] = array("nama"=>$nama,"gambar"=>"$fulldomain/gambar/galeri/$gambar","url"=>$url);
	}
	...
	if ($hlm > 1){
		$prev = $hlm - 1;
		$stringpage[$pageid] = array("nama"=>"Awal","link"=>"$fulldomain/$kanal/$aksi/$katid/$alias/$katid/$alias/1");
		$pageid++;
		$stringpage[$pageid] = array("nama"=>"Sebelumnya","link"=>"$fulldomain/$kanal/$aksi/$katid/$alias/$prev");
	...
	for ($ii=$hlm3; $ii <= $hlm_tot and $ii<=$hlm4; $ii++){
		if ($ii==$hlm){
			...
			$stringpage[$pageid] = array("nama"=>"$ii","link"=>"$fulldomain/$kanal/$aksi/$katid/$alias/$ii");
		}
		$pageid++;
	}
	if ($hlm < $hlm_tot){
		$next = $hlm + 1;
		$stringpage[$pageid] = array("nama"=>"Selanjutnya","link"=>"$fulldomain/$kanal/$aksi/$katid/$alias/$next");
		$pageid++;
		$stringpage[$pageid] = array("nama"=>"Akhir","link"=>"$fulldomain/$kanal/$aksi/$katid/$alias/$hlm_tot");
	}
	else
	{
