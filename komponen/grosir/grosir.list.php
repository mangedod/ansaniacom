...
$sql = "select count(*) as jml from tbl_product where published='1' and grosir='1' $where";
$hsl = sql($sql);
...
$sqlp	= "select produkid,kodeproduk,title,misc_harga,misc_diskon,tag from tbl_product where published='1' and grosir='1' $where order by $sorting desc limit $ord, $judul_per_hlm";
$queryp	= sql($sqlp);
...
while($row2 = sql_fetch_data($queryp))
{
	...
	$savenya		= $misc_matauang." ". number_format($sDiskon,0,".",".");
		...
		if(!empty($gambar_l))
			...
			$image_l	= $lokasiwebtemplate."/images/no_photo.gif";
	
		
		$sql3	= "select albumid,gambar_m,gambar_l from tbl_product_image where produkid='$produkid' and gambar_l!='' order by albumid asc limit 1,1";
		$query3	= sql($sql3);
		$row3	= sql_fetch_data($query3);
		$albumid	= $row3['albumid'];
		$gambar_m	= $row3['gambar_m'];
		...
		if(!empty($gambar_m))
			...
			$image_m1	= $lokasiwebtemplate."/images/no_photo.gif";
			
		if(!empty($gambar_l))
			$image_l1	= "$fulldomain/gambar/produk/$gambar_l";
		else
			$image_l1	= $lokasiwebtemplate."/images/no_photo.gif";
			
	$link_detail	= "$fulldomain/grosir/read/$produkpostid/$alias";
	...
	$list_post[$produkid]	= array("produkid"=>$produkid,"produkpostid"=>$produkpostid,"namaprod"=>$namaprod,"totalrating"=>$totalrating,"image_m"=>$image_m,"image_m1"=>$image_m1,"link_detail"=>$link_detail,"ringkas"=>$ringkas,
