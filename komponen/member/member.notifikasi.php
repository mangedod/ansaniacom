...
if($_SESSION['usernameresel'])
{
	...
	$sql = "select count(*) as jml from tbl_notifikasi where tousername='$_SESSION[usernameresel]'";
	$hsl =sql($sql);
	...
	//Notifikasi
	$perintah = "select id,fromusername,pesan,status,icon,tanggal from tbl_notifikasi where tousername='$_SESSION[usernameresel]' order by tanggal desc limit $ord, $judul_per_hlm";
	$hasil =sql($perintah);
	...
	sql_free_result($hasil);
		...
		if ($hlm > 1){
			...
			$pageid++;
			$stringpage[$pageid] = array("nama"=>"&laquo;","link"=>"$fulldomain/$kanal/$aksi/$prev");
		...
		if ($hlm < $hlm_tot){
			$next = $hlm + 1;
			$stringpage[$pageid] = array("nama"=>"&raquo;","link"=>"$fulldomain/$kanal/$aksi/$next");
			$pageid++;
