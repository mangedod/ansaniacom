<?php 
	...
	else
	{
		...
		elseif($aksi=="syukur")
		{
			$wherex 	= "where jenis='1' ";
		}
		elseif($aksi=="janji")
		{
			$wherex 	= "where jenis='2' ";
		}
		else
		{
			//Untuk Menampilkan yang sudah mengikuti saja
			/*$ids[] = "'$_SESSION[useridresel]'";
			$ids[] = "'0'";
			$res = sql("select fid from tbl_follow where userid='$_SESSION[useridresel]'");
			$x = 0;
	...
	$datapost	= array();
	$perintah	= "select postid,isi,username,userid,userfullname,touserid,tousername,touserfullname,tanggal,jenis,jmlLike,groupnya,type,media,via,jmlkomen from tbl_post $wherex $urut limit $ord, $judul_per_hlm";
	...
	while($data	= sql_fetch_data($hasil))
	{
		...
		if($jmlisi > 500 && $aksi!="post")
		{
			$isi = nl2br(substr($isi,0,500))."... <a href=\"/$usernames/post/$postid/$kanal\">selengkapnya..</a>";
		}
		$isii = explode(" ",$isi);
		...
		$avatarnya = sql_get_var("select avatar from tbl_member where userid='$userid'");
			if ($avatarnya)
				$avatar="$fulldomain/uploads/avatars/$avatarnya";
			else
				$avatar="$lokasiwebtemplate/images/no_pic.jpg";
		...
		if (($usernames == $_SESSION['usernameresel']) or ($tousername == $_SESSION['usernameresel'])) $hapusposting = 1;
		else $hapusposting = 0;
		...
		//cek like atau tidak
		$query2		= "select count(*) as jmllikenya from tbl_post_like where postid='$postid' and username='$_SESSION[usernameresel]'";
		$hhsl		= sql($query2);
		...
		else
		{
			...
			if($aksi=="post")
				...
				$sql	= "select id,username,userid,userfullname,isi,tanggal,via from tbl_post_komen where postid='$postid' order by tanggal asc limit $mulai,2";
			...
			while($row = sql_fetch_data($hsl))
			{
				...
				$avatarnya = sql_get_var("select avatar from tbl_member where userid='$olehuserid'");
					if ($avatarnya)
						$gambar="$fulldomain/uploads/avatars/$avatarnya";
					else
				...
				//hapus komentar nya
				if( ($oleh==$_SESSION['usernameresel']) or ($hapusposting =="1") )
					$hapus	= 1;
				else
					$hapus	= 0;
	...
	if ($hlm > 1){
		$prev = $hlm - 1;
		$stringpage[$pageid] = array("nama"=>"Awal","link"=>"$fulldomain/$kanal2/$aksi/1/$subaksi");
		$pageid++;
		$stringpage[$pageid] = array("nama"=>"&laquo;","link"=>"$fulldomain/$kanal2/$aksi/$prev/$subaksi");
	...
	for ($ii=$hlm3; $ii <= $hlm_tot and $ii<=$hlm4; $ii++){
		if ($ii==$hlm){
			...
			$stringpage[$pageid] = array("nama"=>"$ii","link"=>"$fulldomain/$kanal2/$aksi/$ii/$subaksi");
		}
		$pageid++;
	}
	if ($hlm < $hlm_tot){
		$next = $hlm + 1;
		$stringpage[$pageid] = array("nama"=>"&raquo;","link"=>"$fulldomain/$kanal2/$aksi/$next/$subaksi");
		$pageid++;
		$stringpage[$pageid] = array("nama"=>"Akhir","link"=>"$fulldomain/$kanal2/$aksi/$hlm_tot/$subaksi");
	}
	else
	{
