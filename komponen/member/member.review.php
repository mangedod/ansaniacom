<?php
$nama_aksi 	= "Product Reviews";
$deskripsiaksi 	= "Reviews Of Your Product";
...
$tpl->assign("deskripsiaksi",$deskripsiaksi);
	...
	$abc = "select transaksiid from tbl_transaksi where userid='$_SESSION[userid]' and status='4' order by transaksiid desc limit $ord, $judul_per_hlm";
	$acb = sql($abc);
	$no		= 1;
	...
	while ($bca = sql_fetch_data($acb))
	{
		...
		while($row = sql_fetch_data($qry))
		{
			...
			$sql2	= "select title,misc_harga, misc_diskon from tbl_product_post where produkpostid='$produkpostid'";//secid,subid,misc_harga_reseller,
			$query2	= sql($sql2);
