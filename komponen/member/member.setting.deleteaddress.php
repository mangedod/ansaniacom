<?php
	...
	$query	= "delete from tbl_member_alamat where useralamatid='$useralamatid' and userid='$_SESSION[userid]'";
	$hasil 	= sql($query);
			
	if($hasil)
	{
		$error 		= "Congratulations your data in $title has been successfully deleted.";
		$style		= "alert-success";
		$backlink	= "$fulldomain"."/member/setting";
	}
	else
	{
		$error 		= "Data removal failed there are some errors that must be repaired, please check back.";
		$style		= "alert-danger";
		$backlink	= "$fulldomain"."/member/setting";
	}
