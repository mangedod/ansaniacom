...
if(isset($_POST['submit']))
{
	...
	{
		$userDirName		= $_SESSION[userdirnameresel];
		$date	= date("Ym");
		...
		$namagambarl		= "tema-$date-".$_SESSION['useridresel'].".$ext";
		$namagambars		= "tema-$date-".$_SESSION['useridresel']."-kecil.$ext";
	...
	if($_FILES["header"][size]>0)
	{
		...
		$namagambarl		= "header-$date-".$_SESSION['useridresel'].".$ext";
		$namagambars		= "header-$date-".$_SESSION['useridresel']."-kecil.$ext";
	...
	$sql	= "update tbl_member set tema='$lokasibackground' ,thumbtema='$lokasibackground2', header='$lokasiheader' ,thumbheader='$lokasiheader2' where username='$_SESSION[usernameresel]'";
	$hasil	= sql($sql);
...
$tpl->assign("berhasil",$berhasil);
	...
	$query	= sql("select tema,thumbtema,header,thumbheader from tbl_member where username='$_SESSION[usernameresel]'");
