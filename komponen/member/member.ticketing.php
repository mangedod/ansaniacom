<?php
	...
	$mysql 		= "select ticketingid,secid,judul,create_date,isipertanyaan,status,is_closed,reply_user from tbl_ticketing where userid='$_SESSION[userid]' $where
	...
	while ($data =  sql_fetch_data($hasil))
	{
		...
		$tanggal 			= tanggalonly($create_date);
		
		if ($is_closed == 1)
		{
			$stats	= "<label class=\"label label-success\">The question was already processed.</label>";
		}
		else
		{
			if($reply_user == 2 and $status == 1)
			{
				$stats	= "<label class=\"label label-success\">Have been replied to by Admins.</label>";

			}
			else if($reply_user == 1 and $status == 1)
			{
				$stats	= "<label class=\"label label-danger\">Waiting for a reply from Admins.</label>";
			}
		...
		if(!empty($gambar)) $gambar = "$fulldomain/gambar/ticketing/$gambar";
		else $gambar = "";

		$link 		= "$fulldomain"."/member/detailticketing/$ticketingid/$alias";
	...
	if ($hlm > 1){
		$prev = $hlm - 1;
		$stringpage[$pageid] = array("nama"=>"&laquo;","link"=>"$fulldomain"."/member/ticketing/list/1");
		$pageid++;
		$stringpage[$pageid] = array("nama"=>"&lsaquo;","link"=>"$fulldomain"."/member/ticketing/list/$prev");
	...
	for ($ii=$hlm3; $ii <= $hlm_tot and $ii<=$hlm4; $ii++){
		if ($ii==$hlm){
			...
			$stringpage[$pageid] = array("nama"=>"$ii","link"=>"$fulldomain"."/member/ticketing/list/$ii");
		}
		$pageid++;
	}
	if ($hlm < $hlm_tot){
		$next = $hlm + 1;
		$stringpage[$pageid] = array("nama"=>"&rsaquo;","link"=>"$fulldomain"."/member/ticketing/list/$next");
		$pageid++;
		$stringpage[$pageid] = array("nama"=>"&raquo;","link"=>"$fulldomain"."/member/ticketing/list/$hlm_tot");
	}
	else
	{
