<?php
		...
		$perintah	= "select  transaksiid, userid, orderid, invoiceid, pembayaran, pengiriman, totaltagihan, namalengkap, alamatpengiriman, email, warehouseid, voucherid, kodevoucher, 
					totaldiskon, totaltagihanafterdiskon, ongkosid, ongkoskirim, noresi, status, statuskirim, flag, tanggaltransaksi from tbl_transaksi where 1 $whereb order by tanggaltransaksi desc limit $ord, $judul_per_hlm";// and status > 0
		$hasil		= sql($perintah);
		$datalistrans	= array();
		$no = 0;
		while($data = sql_fetch_data($hasil))
		{
			...
			$no++;
			$datalistrans[$transaksiid] = array("transaksiid"=>$transaksiid,"no"=>$no,"invoiceid"=>$invoiceid,"billingnama"=>$billingnama,"billingalamat"=>$billingalamat,
												"billingemail"=>$billingemail,"totaltagihan"=>$totaltagihan,"total_diskon"=>$total_diskon,"ongkos_kirim"=>$ongkos_kirim,
												"total_bayar"=>$total_bayar,"pembayaran"=>$pembayaran,"tanggaltransaksi"=>$tanggaltransaksi,"urldetail"=>$link);
		}
		sql_free_result($hasil);
		print_r($datalistrans);
