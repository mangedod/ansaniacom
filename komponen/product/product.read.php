<?php 
		...
		$sql           = "select produkpostid, jenisid, secid, subid, kodeproduk, title, content, ringkas, misc_harga, cart, misc_diskon, create_date,sizeid,body_weight,body_dimension, misc_matauang, brandid, tag from tbl_product_post where status='1' and produkpostid='$produkpostid' and published='1'";
		$query         = sql($sql);
		$row           = sql_fetch_data($query);
		$produkid      = $row["produkpostid"];
		...
		$sql	= "update tbl_product_post set numviews=numviews+1 where kodeproduk='$kodeproduk' and produkid='$produkid'";
		$query	= sql($sql);
		...
		// Gambar
		$sql3		= "select albumid,nama,gambar_m,gambar_s,gambar_l,gambar_f,produkpostid from tbl_product_image where produkpostid='$produkpostid' order by utama desc, albumid asc";
		$query3		= sql($sql3);
		$list_image	= array();
		$ii			= 1;
		$albumarr 	= array();
		...
		if($_POST['aksi'] == "simpan")
		{
			...
			else
			{
				...
				$perintah	= "insert into tbl_product_comment (produkpostid,userid,nama,email,score,komentar,ip,published,via,create_date,create_userid) 
							values ('$produkpostid',$userid','$_SESSION[userfullname]','$_SESSION[useremail]','$score','$komentar','$ip','1','".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']."','$date','$cuserid')";
							
				$hasil		= sql($perintah);
				
			
				if($hasil)
					header("location: $fulldomain/product/read/$aliasSec/$aliasSub/$produkpostid$callback");
			}
		}
		
		// list comment
		$sqlc	= "select commentid,nama,komentar,create_date,userid,score from tbl_product_comment where produkpostid='$produkpostid' and published='1' order by create_date desc";//produkpostid='$produkpostid' and 
		$queryc	= sql($sqlc);
		...
		$sql2	= "select kodeproduk,produkpostid,secid,subid,title,misc_harga,ringkas, misc_matauang,misc_diskon from tbl_product_post where status='1' and published='1' and produkpostid!='$produkpostid' and jenisid='$jenisid' order by update_date desc limit 8";
		$query2	= sql($sql2);
		...
		while($row2 = sql_fetch_data($query2))
		{
			...
			$sql3	= "select albumid,gambar_s,gambar_m from tbl_product_image where produkpostid='$produkid1' and utama='1' order by albumid asc  limit 1";// and produkid='$produkid'
			$query3	= sql($sql3);
			...
			if(!empty($gambar_m))
				$image_m	= "$fulldomain/gambar/produk/$produkid1/$gambar_m";
			else
				$image_m	= $fulldomain.$lokasiwebtemplate."images/no_photo.gif";

			$sql3	= "select albumid,gambar_s,gambar_m from tbl_product_image where produkpostid='$produkid1' and utama='1' order by albumid asc limit 1,1";// and produkid='$produkid'
			$query3	= sql($sql3);
			$row3	= sql_fetch_data($query3);
			$albumid	= $row3['albumid'];
			$gambar_s	= $row3['gambar_s'];
			...
			if(!empty($gambar_m))
				$image_m1	= "$fulldomain/gambar/produk/$produkid1/$gambar_m";
			else
				$image_m1	= $fulldomain.$lokasiwebtemplate."images/no_photo.gif";
			
		
			$link_detail	= "$fulldomain/product/read/$produkid1/$aliasprod/";
			$link_buy 		= "$fulldomain"."quickbuy/addpost/$produkid1/$aliasprod/";
			
			$produklain[$produkid1]	= array("produkid1"=>$produkid1,"produkpostid"=>$produkpostid1,"namaprod"=>$namaprod,"image_m"=>$image_m,"image_m1"=>$image_m1,"link_buy"=>$link_buy,
