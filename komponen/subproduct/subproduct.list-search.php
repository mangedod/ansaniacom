...
if($_POST['aksi'] == "cari" or preg_match("/search/i",$aksi))
{
	...
	if($_POST['aksi'] == "cari")
	{
		unset($_SESSION['secid'],$_SESSION['subid'],$_SESSION['brandid'],$_SESSION['kata'],$_SESSION['idprod']);
	...
	if($idprods != "")
		$where	.= " and produkpostid='$idprods'";
	if($kata != "")
	{
		$tempk	= explode(" ",$kata);
...
$sqlp	= "select produkpostid,secid,subid,brandid,title,ringkas,misc_harga,misc_diskon,misc_matauang,pilihan,misc_harga_reseller,icon from tbl_product_post where status='1' 
		and published='1' $where order by $sorting desc limit $ord, $judul_per_hlm";
$queryp	= sql($sqlp);
