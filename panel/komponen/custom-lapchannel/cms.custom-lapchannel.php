<?php
	...
	if($aksi=="view")
	{
		...
		$qry = "select channelid,nama from tbl_channel where 1";
		$hsl=sql($qry);
		while($data=sql_fetch_data($hsl))
		{
			$dataselect2[] = array("$data[channelid]","$data[nama]");
		}
		
		$cari[] = array("channelid","Channel","select","select",$dataselect2);
		...
		{	
			...
			//transaksi
			$jml 	= sql_get_var("select count(*) as jumlah from  $nama_tabel where tanggaltransaksi='$tanggal' and channelid='$channelid'");
			$jmlinv	= sql_get_var("select sum(totalinvoice) as jumlah from  $nama_tabel where tanggaltransaksi='$tanggal' and channelid='$channelid'");
