...
$nama_tabel		= "tbl_customdesigngaleri_sec";
$nama_tabel1	= "tbl_customdesigngaleri";
$judul_per_hlm 	= 25;
...
else
{
	...
	if($aksi=="view")
	{
        ...
        {
			$mainmenu[] = array("Lihat Galeri","lihat","$alamat&aksi=view");
			...
			$sql = "select id,nama,ringkas,published,gambar from $nama_tabel1  where 1 $where $parorder limit $ord, $judul_per_hlm";
			$hsl = sql($sql);
			$i = 1;
			$a = 1;
			...
			while ($row = sql_fetch_data($hsl))
			{
				...
				print("<tr class=\"row$i\"><td width=5% height=20 valign=top>&nbsp;$a</td>
					...
					<td  valign=top class=judul><b><a href=\"$alamat&aksi=detailgaleri&id=$id&hlm=$hlm\">$nama</a></b><br clear=\"all\" /> $ringkas</td>\n
					<td  valign=top >$publish</td>");

				print("<td>");

				if($published==0) $acc[] = array("Publish","push","$alamat&aksi=publishgaleri&id=$id&hlm=$hlm");
				else $acc[] = array("Unpublish","push","$alamat&aksi=publishgaleri&id=$id&hlm=$hlm");

				$acc[] = array("Detail","detail","$alamat&aksi=detailgaleri&id=$id&hlm=$hlm");
				$acc[] = array("Edit","edit","$alamat&aksi=editgaleri&id=$id&hlm=$hlm");
				$acc[] = array("Hapus","delete","$alamat&aksi=hapusgaleri&id=$id&hlm=$hlm");
	...
	if($aksi=="detailgaleri")
	{
		...
		else
        {
			...
			$sql = "select id,nama,ringkas,oleh,alias,gambar,gambar1,create_date,create_userid,update_date,update_userid from $nama_tabel1  where id='$id'";
			$hsl = sql($sql);
			...
			<table border="0" class="tabel-cms" width="100%">
				...
				<tr>
					<td valign="top">&nbsp;</td>
					<td align="left">
						<input type="button" onclick="javascript:window.location='<?php echo "$alamat&aksi=editgaleri&id=$id"?>'" value="Ubah Data">
					</td>
				</tr>
			</table>
			</form>
	...
	if($aksi=="hapusgaleri")
	{
		...
		else
        {
			...
			$id       = $_GET['id'];
			$perintah = "select gambar,gambar1,customdesignid from $nama_tabel1 where id='$id'";
			$hasil    = sql($perintah);
			...
			$perintah = "delete from $nama_tabel1 where id='$id' and customdesignid='$customdesignid'";
			$hasil = sql($perintah);

			if($hasil)
			{
				$msg = base64_encode("Success menghapus galeri dengan ID $id");
	...
	if($aksi=="publishgaleri")
	{
		...
		else
        {
			...
			$perintah 	= "select published from $nama_tabel1 where id='$id'";
			$hasil 		= sql($perintah);
			...
			$perintah 	= "update $nama_tabel1 set published='$status' where id='$id'";
			$hasil		= sql($perintah);
	...
	if($aksi=="savetambahgaleri")
	{
		...
		else
        {
			...
			for($x=0;$x<$xx;$x++)
			{
				...
				$perintah = "insert into $nama_tabel1(id,nama,alias,ringkas,oleh,create_date,create_userid,published $fgambar)
							values ('$new','$nama','$alias','$ringkas','$oleh','$date','$cuserid','1' $vgambar)";
				$hasil = sql($perintah);
	...
	if($aksi=="saveeditgaleri")
	{
		...
		else
        {
			...
			$perintah = "update $nama_tabel1 set nama='$nama',alias='$alias',ringkas='$ringkas',
						update_date='$date',update_userid='$cuserid' $vgambar where id='$id' and customdesignid='$customdesignid'";
			$hasil = sql($perintah);
	...
	if($aksi=="editgaleri")
	{
		...
		else
        {
			...
			$sql = "select id,nama,ringkas,customdesignid from $nama_tabel1  where id='$id'";
			$hsl = sql($sql);
			$row = sql_fetch_data($hsl);
