...
else
{
	...
	if($aksi=="view")
	{
        ...
        {
			$mainmenu[] = array("Lihat Header Image","lihat","$alamat&aksi=view");
			$mainmenu[] = array("Tambah Header Image","tambah","$alamat&aksi=tambah");
			...
			$sql = "select id,nama,url,gambar,published from $nama_tabel  where 1 $where $parorder limit $ord, $judul_per_hlm";
			$hsl = sql($sql);
			$i = 1;
			$a = 1;
	...
	if($aksi=="savetambah")
	{
		...
		else
        {
			...
			$perintah = "insert into $nama_tabel(id,nama,url,create_date,create_userid $fgambar) 
						values ('$new','$nama','$url','$date','$cuserid' $vgambar)";
			$hasil = sql($perintah);
	...
	if($aksi=="tambah")
	{
		...
		else
        {
			...
			<table border="0" class="tabel-cms" width="100%">
				<tr>
					<th colspan="2">Tambah Image</th>
				</tr>
				<tr> 
					<td width="15%">Nama Image</td>
					<td ><input name="nama" type="text" size="50" value="" id="nama" class="validate[required]" /></td>
				</tr>
                 <tr> 
					<td >Gambar</td>
					<td><input name="gambar" type="file" size="20" value="" id="gambar" title="Pilih Gambar Dari Drive" class="file" /> <em>Ukuran Gambar 562 x 168 pixel</em></td>
				</tr>
				<tr> 
					<td valign="top">&nbsp;</td>
					<td align="left">
	...
	if($aksi=="saveedit")
	{
		...
		else
        {
			...
			$perintah = "update $nama_tabel set nama='$nama',url='$url',
	...
	if($aksi=="edit")
	{
		...
		else
        {
			...
			<table border="0" class="tabel-cms" width="100%">
                ...
                <tr> 
					<td >Gambar</td>
					<td><input name="gambar" type="file" size="20" value="" id="gambar" title="Pilih Gambar Dari Drive" /> <em>Ukuran Gambar 562 x 168 pixel</em></td>
				</tr>
				<tr> 
					<td valign="top">&nbsp;</td>
					<td align="left">
