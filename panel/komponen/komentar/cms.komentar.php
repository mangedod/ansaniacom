...
else
{
	...
	if($aksi=="view")
	{
        ...
        {
			$mainmenu[] = array("Lihat Komentar","lihat","$alamat&aksi=view");
			...
			$sql = "select komid,kanal,id,nama,email,ip,komentar,published from $nama_tabel  where 1 $where $parorder limit $ord, $judul_per_hlm";
			$hsl = sql($sql);
			$i = 1;
			$a = 1;
			...
			print("<th width=20%><a href=\"$urlorder&order=nama\" title=\"Urutkan\">Komentator</a></th>\n");
			print("<th width=60%>Komentar</th>\n");
			print("<th width=20%>Content</th>\n");
			...
			while ($row = sql_fetch_data($hsl))
			{
				...
				if($kanals=="berita" || $kanals=="artikel" || $kanals=="video")
				{ 
					$sqlj = sql("select nama,alias,secid from tbl_$kanals where id='$id'");
					$dt = sql_fetch_data($sqlj);
					$judul = $dt['nama'];
					$alias = $dt['alias'];
					$secid = $dt['secid'];
					$urls = "$fulldomain/$kanals/detail/$secid/$id/$alias#komentar";
				}
				else
				{
					$sqlj = sql("select nama,alias from tbl_$kanals where id='$id'");
					$dt = sql_fetch_data($sqlj);
					$judul = $dt['nama'];
					$alias = $dt['alias']; 
					$urls = "$fulldomain/$kanals/detail/$secid/$id/$alias#komentar";
