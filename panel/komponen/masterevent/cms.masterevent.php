...
else
{
	...
	if($aksi=="view")
	{
        ...
        {
			$mainmenu[] = array("Lihat Master Acara","lihat","$alamat&aksi=view");
			$mainmenu[] = array("Tambah Master Acara","tambah","$alamat&aksi=tambah");
			mainaction($mainmenu,$pageparam);
			
			//Search Paramater Fungsi namafield,Label,
			$cari[] = array("nama","Acara","str","text","$data");
			...
			$sql = "select id,nama,keterangan from $nama_tabel  where 1 $where $parorder limit $ord, $judul_per_hlm";
			$hsl = sql($sql);
			$i = 1;
			$a = 1;
	...
	if($aksi=="savetambah")
	{
		...
		else
        {
			...
			$perintah = "insert into $nama_tabel(id,nama,alias,keterangan,create_date,create_userid) 
	...
	if($aksi=="edit")
	{
		...
		else
        {
			...
			$sql = "select id,nama,keterangan from $nama_tabel  where id='$id'";
			$hsl = sql($sql);
