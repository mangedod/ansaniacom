...
else
{
	...
	if($aksi=="view")
	{
		if(!$oto['view']) { echo $error['view']; } else
		{
			...
			while ($row = sql_fetch_data($hsl))
			{
				...
				$acc[] = array("Send Testing","mail","$alamat&aksi=testing&newsletterid=$newsletterid&hlm=$hlm");
	...
	if($aksi=="detail")
	{
		if(!$oto['view']) { echo $error['view']; } else
		{
			...
			<table border="0" class="tabel-cms" width="100%">
                ...
                <tr>
					<td class="tdinfo">HTMl Newsletter</td>
					<td><iframe width="100%" height="700" style="border:1px solid #CCCCCC" src="<?php echo "komponen/$kanal/previewhtml.php?newsletterid=$newsletterid";?>"></iframe></td>
				</tr>
				<tr>
					<td valign="top">&nbsp;</td>
					<td align="left">
	...
	if($aksi=="savetambah")
	{
		if(!$oto['add']) { echo $error['add']; } else
		{
			...
			$perintah = "insert into $nama_tabel(newsletterid,newslettername,subject,voucherid,plainmail,htmlmail,senddate,sendername,create_date,create_userid)
						values ('$new','$newslettername','$subject','$voucherid','$plainmail','$htmlmail','$senddate','$sendername','$date','$cuserid')";
			$hasil = sql($perintah);
			...
			<table border="0" class="tabel-cms" width="100%">
                ...
                <tr>
                 ...
                 <tr>
					...
					<td >Untuk mengirimkan newseletter dengan data pelanggan atau voucher silahkan sematkan --userfullname-- di dalam email untuk Nama lengkap, --useremail-- untuk
	...
	if($aksi=="saveedit")
	{
		...
		if(!$oto['edit']) { echo $error['edit']; } else
		{
			...
			$perintah = "update $nama_tabel set newslettername='$newslettername', voucherid='$voucherid',subject='$subject', plainmail='$plainmail', htmlmail='$htmlmail', tags='$tags', subaccount='$subaccount', sender='$sender', sendername='$sendername', senddate='$senddate',update_date='$date',update_userid='$cuserid' $vgambar where newsletterid='$newsletterid'";
			$hasil = sql($perintah);
	...
	if($aksi=="edit")
	{
		if(!$oto['edit']) { echo $error['edit']; } else
		{
			...
			$sql = "select newsletterid,subject,plainmail,htmlmail,newslettername,voucherid,senddate,tags,subaccount,sender,sendername from $nama_tabel  where newsletterid='$newsletterid'";
			$hsl = sql($sql);
			$row = sql_fetch_data($hsl);
			...
			<table border="0" class="tabel-cms" width="100%">
                ...
                <tr>
					<td >HTML Newsletter</td>
					<td ><textarea name="htmlmail" cols="70" rows="10" id="htmlmail" class="ckeditor" ><?php echo $htmlmail?></textarea></td>
				</tr>
...
WHERE b.produkid IN (1,2,3,4,5) and b.produkid != 0 GROUP BY a.memberid";
			...
			<table border="0" class="tabel-cms" width="100%">
				...
				<tr class="bygroup" style="display:none">
					...
					<td ><select name="groupid">
                        ...
                        <?php
							$sql = "select groupid, nama from tbl_subscriber_group";
							$qry = sql($sql);
		...
		if(!$oto['add']) { echo $error['add']; } else
		{
			...
			<table border="0" class="tabel-cms" width="100%">
				...
				<tr>
					<td valign="top">&nbsp;</td>
					<td align="left">
						<input type="submit" name="Submit" value="Testing Kirim Email" />
						<input type="button" onclick="javascript:history.back();" value="Batal">
					</td>
				</tr>
			</table>
	...
	if($aksi=="savetesting")
	{
		...
		/*	//GET Configuration
			...
			if(sendmail($penerima,$email,$subject,$plainmail,$htmlmail))
			{
				$msg = base64_encode("Email Berhasil dikirimkan");
				header("location: $alamat&aksi=view&newsletterid=$newsletterid&hlm=$hlm&msg=$msg");
				exit();
			}
			else
			{
				$error = base64_encode("Email Gagal dikirimkan");
				header("location: $alamat&aksi=view&newsletterid=$newsletterid&hlm=$hlm&error=$error");
				exit();
