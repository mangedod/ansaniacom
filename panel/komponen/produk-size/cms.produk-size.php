...
//Variable Umum
if(isset($_POST['sizeid'])) $sizeid = $_POST['sizeid'];
 else $sizeid = $_GET['sizeid']; 
...
else
{
	...
	if($aksi=="view")
	{
		...
		else
        {
			...
			$cari[] = array("nama","Ukuran","str","text","$data");
			...
			//Orderring
			$order = getorder("sizeid","desc",$pageparam,$param);
			$parorder = $order[0];
			$urlorder = $order[1];	
			...
			$sql = "select sizeid,nama,keterangan from $nama_tabel where 1 $where $parorder limit $ord, $judul_per_hlm";
			$hsl = sql($sql);
			$i = 1;
			$a = 1;
			...
			print("<th width=30%><a href=\"$urlorder&order=nama\" title=\"Urutkan\">Ukuran</a></th>\n");
			print("<th width=60%>Keterangan</th>\n");
			...
			while ($row = sql_fetch_data($hsl))
			{
				...
				$acc[] = array("Edit","edit","$alamat&aksi=edit&sizeid=$sizeid&hlm=$hlm");
				$acc[] = array("Hapus","delete","$alamat&aksi=hapus&sizeid=$sizeid&hlm=$hlm");
	...
	if($aksi=="hapus")
	{
		...
		else
        {
			...
			$perintah 	= "delete from $nama_tabel where sizeid='$sizeid'";
			$hasil 		= sql($perintah);

			if($hasil)
			{ 
				// setlog($_SESSION['cms_userid'],"$_SESSION[cms_userfullname] melakukan Penghapusan data Kategori Produk dengan ID $id",$uri,$ip);  
				$msg = base64_encode("Success menghapus menu dengan Data dengan ID $sizeid");
	...
	if($aksi=="savetambah")
	{
		...
		else
        {
			...
			$new 		= newid("sizeid",$nama_tabel);
			
			$perintah 	= "insert into $nama_tabel(sizeid,nama,keterangan) 
	...
	if($aksi=="saveedit")
	{
		...
		else
        {
			...
			$perintah 	= "update $nama_tabel set nama='$nama',keterangan='$keterangan' where sizeid='$sizeid'";
			$hasil 		= sql($perintah);

			if($hasil)
			{  
				// setlog($_SESSION['cms_userid'],"$_SESSION[cms_userfullname] melakukan perubahan data Kategori Produk dengan ID $id",$uri,$ip); 
				$msg = base64_encode("Berhasil mengubah data dengan ID $sizeid");
				header("location: $alamat&aksi=view&hlm=$hlm&msg=$msg");
				exit();
			}
			else
	...
	if($aksi=="edit")
	{
		...
		else
        {
			...
			$sql = "select sizeid,nama,keterangan from $nama_tabel where sizeid='$sizeid'";
			$hsl = sql($sql);
	...
	if($aksi=="popidsize")
	{
		...
		else
        {
			...
			?>
				...
				function pushdata(sizeid,nama)
				{
					 if (window.opener && !window.opener.closed)
					 {
					 	window.opener.$("#<?php echo $var1; ?>").val(sizeid);
			...
			$sql = "select nama,sizeid,keterangan from $nama_tabel where 1 $where $parorder limit $ord, $judul_per_hlm";
			$hsl = sql($sql);
			$i = 1;
			$a = 1;
			...
			print("<th width=30%><a href=\"$urlorder&order=nama\" title=\"Urutkan\">Ukuran</a></th>\n");
			print("<th width=50%>Keterangan</th>\n");
			...
			while ($row = sql_fetch_data($hsl))
			{
				...
				print("<td><button class=\"btn dropdown-toggle\" onclick=\"pushdata('$sizeid','$nama');\">Select</button>");
				print("</td></tr>");
