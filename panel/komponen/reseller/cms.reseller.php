...
else
{
	...
	if($aksi=="view")
	{
		if(!$oto['view']) { echo $error['view']; }  
			...
			$mainmenu[] = array("Lihat Reseller","lihat","$alamat&aksi=view");
			$mainmenu[] = array("Tambah Reseller","tambah","$alamat&aksi=tambahadmin");
			...
			$sql = "select count(*) as jml from $nama_tabel where tipe='1' $where $parorder";
			$hsl = sql($sql);
			...
			$sql = "select userid,username,tipe,userfullname,useremail,verified,pilihan,avatar,userphonegsm from $nama_tabel  where tipe='1' $where $parorder limit $ord, $judul_per_hlm";
			$hsl = sql($sql);
			$i = 1;
			$a = 1;
			...
			print("<th width=20%><a href=\"$urlorder&order=userphonegsm\" title=\"Urutkan\">Handphone</a></th>");
			// print("<th width=10%><a href=\"$urlorder&order=tipe\" title=\"Urutkan\">Tipe</a></th>");
			print("<th width=10%><a href=\"$urlorder&order=verified\" title=\"Urutkan\">Verified</a></th>");
			...
			while ($row = sql_fetch_data($hsl))
			{
				...
				print("<td valign=top class=hitam>$userphonegsm</td>\n");
				print("<td align=center><a href=\"$alamat&aksi=verified&userid=$userid\">$verified</a></td>");
	...
	if($aksi=="popupuserid")
	{
		...
		else
        {
			...
			$where = $formcari[0]['where']."and tipe='1'";
			$param = $formcari[0]['param'];
			...
			else
			{
				...
				$perintah = "INSERT INTO $nama_tabel (userid,tipe,username,useremail,userpassword,userfullname,usergender,userdob,useraddress,kotaid,cityname,
							propinsiid,negaraid,userpostcode,userphone,userphonegsm,useractivestatus,userlastloggedin,userlastactive,userstatus,usercreateddate $avatar1)
							 VALUES ('$baru','1','$username','$useremail','$userpassword','$userfullname','$usergender','$userdob','$useraddress','$kotaid','$cityname',
				...
				{   
					...
					$msg = base64_encode("Berhasil ditambahkan Reseller baru dengan username = $username");
					header("location: $alamat&aksi=view&hlm=$hlm&msg=$msg");
					exit();
				}
				else
	...
	if($aksi=="tambahadmin")
	{
		if(!$oto['add']) { echo $error['add']; }
		{
			...
			<table border="0" class="tabel-cms" width="100%">
				<tr>
					<th colspan="2">Tambah Reseller</th>
				</tr>
                <tr> 
					<td width="176">Username</td>
				...
				<tr> 
					<td valign="top">&nbsp;</td>
					<td align="left">
						<input type="submit" name="Submit" value="Simpan Reseller" />
	...
	if($aksi=="saveeditadmin")
	{
		...
		if(!$oto['edit']) { echo $error['edit']; }
		{
			...
			$perintah = "update $nama_tabel set tipe='1',refuserid='$refuserid',userfullname='$userfullname',usergender='$usergender',userdob='$userdob',
			...
			if($hasil)
			{   
				$msg = base64_encode("Berhasil mengubah Data Reseller dengan username = $username");
				header("location: $alamat&aksi=view&hlm=$hlm&msg=$msg");
				exit();
			}
			else
	...
	if($aksi=="edit")
	{
		if(!$oto['edit']) { echo $error['edit']; }
		{
            ...
            <table border="0" class="tabel-cms" width="100%">
				<tr>
					<th colspan="2">Ubah Data Reseller</th>
				</tr>
                <tr> 
					<td width="176">Username</td>
