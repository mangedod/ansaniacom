...
else
{
	...
	if($aksi=="view")
	{
		...
		{
			$mainmenu[] = array("Lihat SMS Keyword","lihat","$alamat&aksi=view");
			$mainmenu[] = array("Tambah SMS Keyword","tambah","$alamat&aksi=tambah");
			mainaction($mainmenu,$pageparam);

			//Search Paramater Fungsi namafield,Label,
			$cari[]       = array("waktu","Waktu Pengiriman","date","date","$data");
			...
			$tot = sql_get_var("select count(*) as jml from $nama_tabel where jenis='keyword' $where $parorder");
			...
			$sql = "select id,isipesan,keyword,jenis,published from $nama_tabel  where jenis='keyword' $where $parorder limit $ord, $judul_per_hlm";
			$hsl = sql($sql);
			$i	 = 1;
			...
			echo "<th width=25%><a href='$urlorder&order=keyword' title='Urutkan'>Keyword</a></th>";
			echo "<th width=45%>Isi Pesan</th>";
	...
	if($aksi=="hapus")
	{
		...
		{
			$sql = "delete from $nama_tabel where id='$id' and jenis='keyword'";
			$hsl = sql($sql);
			if($hsl)
			{
	...
	if ($aksi == "publish") 
	{
		...
		if(!$oto['edit']) { echo $error['edit']; } else 
		{
			...
			$perintah 	= "select published from $nama_tabel WHERE id='$id' and jenis='keyword'";
			$hasil 		= sql($perintah);
			...
			$perintah 	= "update $nama_tabel SET `published`='$status' WHERE id='$id' and jenis='keyword'";
			$hasil 		= sql($perintah);
	...
	if($aksi=="savetambah")
	{
		if(!$oto['add']) { echo $error['add']; } else 
		{
			...
			$sql = "INSERT INTO $nama_tabel (id,create_date,create_userid,keyword,isipesan,jenis) 
					VALUES ('$new','$date','$cuserid','$keyword','$isipesan','keyword')";
			$hsl = sql($sql);
		...
		if(!$oto['add']) { echo $error['add']; } else 
		{
			...
			<form method="post" name="menufrm" id="menufrm">
				...
				<table border="0" class="tabel-cms" width="100%">
					<tr>
						<th colspan="2">Tambah SMS Keyword</th>
					</tr>
					<tr> 
						<td valign="top">Keyword</td>
						<td align="left">
	                    	<input name="keyword" type="text" size="20" id="keyword" class="validate[required]" />
	                  	</td>
					</tr>
					<tr>
						<td>Isi SMS</td>
	...
	if($aksi=="saveedit")
	{
		if(!$oto['edit']) { echo $error['edit']; } else 
		{
			...
			$sql = "update $nama_tabel set update_date='$date',update_userid='$cuserid',
				keyword='$keyword', isipesan='$isipesan' where id='$id' and jenis='keyword'";
			$hsl = sql($sql);
			if($hsl)
			{
				$msg = base64_encode("Success merubah data dengan ID $id");
	...
	if($aksi=="edit")
	{
		if(!$oto['edit']) { echo $error['edit']; } else 
		{
			...
			$sql = "select keyword,isipesan from $nama_tabel  where id='$id' and jenis='keyword'";
			$hsl = sql($sql);
			...
			<form method="post" name="menufrm" id="menufrm">
				...
				<table border="0" class="tabel-cms" width="100%">
					<tr>
						<th colspan="2">Edit SMS Keyword</th>
					</tr>
					<tr> 
						<td valign="top">Keyword</td>
						<td align="left">
	                    	<input name="keyword" value="<?php echo $keyword?>" type="text" size="20" id="keyword" class="validate[required]" />
	                  	</td>
					</tr>
					<tr>
						<td>Isi SMS</td>
