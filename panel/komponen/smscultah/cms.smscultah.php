...
$nama_tabel		= "tbl_smsc_smsterjadwal";
$judul_per_hlm 	= 10;
...
else
{
	...
	if($aksi=="view")
	{
		...
		{
			$mainmenu[] = array("Lihat SMS Ulang Tahun","lihat","$alamat&aksi=view");
			$mainmenu[] = array("Tambah SMS Ulang Tahun","tambah","$alamat&aksi=tambah");
			...
			$tot = sql_get_var("select count(*) as jml from $nama_tabel where jenis='ultah' $where $parorder");
			...
			$sql = "select id,isipesan,jenis,published from $nama_tabel  where jenis='ultah' $where $parorder limit $ord, $judul_per_hlm";
			$hsl = sql($sql);
			$i	 = 1;
	...
	if($aksi=="hapus")
	{
		...
		{
			$sql = "delete from $nama_tabel where id='$id' and jenis='ultah'";
			$hsl = sql($sql);
			if($hsl)
			{
	...
	if ($aksi == "publish") 
	{
		...
		if(!$oto['edit']) { echo $error['edit']; } else 
		{
			...
			$perintah 	= "select published from $nama_tabel WHERE id='$id' and jenis='ultah'";
			$hasil 		= sql($perintah);
			...
			$perintah 	= "update $nama_tabel SET `published`='$status' WHERE id='$id' and jenis='ultah'";
			$hasil 		= sql($perintah);
	...
	if($aksi=="savetambah")
	{
		if(!$oto['add']) { echo $error['add']; } else 
		{
			...
			$sql = "INSERT INTO $nama_tabel (id,create_date,create_userid,isipesan,jenis) 
					VALUES ('$new','$date','$cuserid','$isipesan','ultah')";
			$hsl = sql($sql);

			if($hsl)
			{
				updatenum($no_workorder);
		...
		if(!$oto['add']) { echo $error['add']; } else 
		{
			...
			<form method="post" name="menufrm" id="menufrm">
				...
				<table border="0" class="tabel-cms" width="100%">
					<tr>
						<th colspan="2">Tambah SMS Ulang Tahun</th>
					</tr>
					<tr>
						<td>Isi SMS</td>
	...
	if($aksi=="saveedit")
	{
		if(!$oto['edit']) { echo $error['edit']; } else 
		{
			...
			$sql = "update $nama_tabel set update_date='$date',update_userid='$cuserid',
				isipesan='$isipesan' where id='$id' and jenis='ultah'";
			$hsl = sql($sql);
			if($hsl)
			{
				$msg = base64_encode("Success merubah data dengan ID $id");
	...
	if($aksi=="edit")
	{
		if(!$oto['edit']) { echo $error['edit']; } else 
		{
			...
			$sql = "select isipesan from $nama_tabel  where id='$id' and jenis='ultah'";
			$hsl = sql($sql);
			...
			<form method="post" name="menufrm" id="menufrm">
				...
				<table border="0" class="tabel-cms" width="100%">
					<tr>
						<th colspan="2">Edit SMS Ulang Tahun</th>
					</tr>
					<tr>
						<td>Isi SMS</td>
						<td><textarea style="width:400px;height:70px" rows="3" name="pesan" class="validate[required]" onKeyDown="textCounter(document.menufrm.pesan,document.menufrm.inputcount,130);" onKeyUp="textCounter(document.menufrm.pesan,document.menufrm.inputcount,130);"><?php echo $isipesan?></textarea></td>
