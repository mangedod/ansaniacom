...
else
{
	...
	if($aksi=="view")
	{
		...
		else
        {
			...
			$sql = "select id,nama,ringkas,lengkap,oleh,alias,telp,gsm,bbm,gambar,gambar1,create_date,create_userid,update_date,update_userid from $nama_tabel  where alias='$kanal'";
			$hsl = sql($sql);
	...
	if($aksi=="saveedit")
	{
		...
		else
        {
			...
			$perintah = "update $nama_tabel set nama='$nama',ringkas='$ringkas',lengkap='$lengkap',oleh='$oleh',telp='$telp',gsm='$gsm',bbm='$bbm',
	...
	if($aksi=="edit")
	{
		...
		else
        {
			...
			$sql = "select id,nama,ringkas,lengkap,oleh,telp,gsm,bbm from $nama_tabel  where alias='$kanal'";
			$hsl = sql($sql);
			$row = sql_fetch_data($hsl);
