...
if (!$oto['oto']) { echo $error['oto']; } else
{
	...
	if($aksi=="view")
	{
		if (!$oto['view']) { echo $error['view']; } else
        {
			...
			print("<th width=10%><a href=\"$urlorder&order=userid\" title=\"Urutkan\">Pengirim</a></th>\n");
			print("<th width=35%>Pesan</th>");
			print("<th width=15%>Kategori</th>");
			...
			while ($row = sql_fetch_data($hsl))
			{
				...
				else
				{
					if($reply_user == 2 and $balas == 1)
					{
						...
						$closed		= "<img src='./template/images/unlock.png'> <a href=\"$alamat&aksi=closed&id=$id&secid=$secid&hlm=$hlm\"><strong>Open</strong></a>";
						$reply		= "<img src='./template/images/email_reply.png'> <a href=\"$alamat&aksi=reply&id=$id&secid=$secid&hlm=$hlm\"><strong>Reply</strong></a>";

					}
					else if($reply_user == 1 and $balas == 1)
					{
						$keterangan	= "<img src='./template/images/exclamation.png'> <span style='background:#1f1f9e; color:#fff'>Dibalas oleh Konsumen dan sedang diproses</span>";
						$closed		= "<img src='./template/images/unlock.png'> <a href=\"$alamat&aksi=closed&id=$id&secid=$secid&hlm=$hlm\"><strong>Open</strong></a>";
						$reply		= "<img src='./template/images/email_reply.png'> <a href=\"$alamat&aksi=reply&id=$id&secid=$secid&hlm=$hlm\"><strong>Reply</strong></a>";
				...
				print("<td valign=top><a href=\"$alamat&aksi=detail&id=$id&secid=$secid&hlm=$hlm\"><strong>$judul</strong></a>
				...
				if($close==0)
				{
					if (($_SESSION['cms_gid']==2) or ($_SESSION['cms_isticketing']==1)) $acc[] = array("Reply","edit","$alamat&aksi=reply&id=$id&secid=$secid&hlm=$hlm");
					if (($_SESSION['cms_gid']==2) or ($_SESSION['cms_isticketing']==1)) $acc[] = array("Edit","edit","$alamat&aksi=edit&id=$id&secid=$secid&hlm=$hlm");
				}
				if (($_SESSION['cms_gid']==2) or ($_SESSION['cms_isticketing']==1)) $acc[] = array("Hapus","delete","$alamat&aksi=hapus&id=$id&secid=$secid&hlm=$hlm");
	...
	if($aksi=="detail")
	{
		if (!$oto['view']) { echo $error['view']; } else
        {
			...
			<?php
			$sql	= "select create_userid,isijawaban,create_date,ticketingbalasid,filejawab from $nama_tabel1 where ticketingid='$id'";
			$hsl 	= sql($sql);
			$i 		= 1;
			$a 		= 1;	
			...
			print("<td width=10%><a href=\"$urlorder&order=create_userid\" title=\"Urutkan\"><strong>Tutor</strong></a></td>\n");
			...
			while ($row = sql_fetch_data($hsl))
			{
				...
				if($close==0)
				{
					if (($_SESSION['cms_gid']==2) or ($_SESSION['cms_isticketing']==1)) $acc[] = array("Edit","edit","$alamat&aksi=editreply&id=$id&secid=$secid&balasid=$balasid&hlm=$hlm");
				}
				if (($_SESSION['cms_gid']==2) or ($_SESSION['cms_isticketing']==1)) $acc[] = array("Hapus","delete","$alamat&aksi=hapusreply&id=$id&secid=$secid&balasid=$balasid&hlm=$hlm");
	...
	if($aksi=="hapusreply")
	{
		...
		else
        {
			...
			if($hasil)
			{
				...
				header("location: $alamat&aksi=detail&id=$id&secid=$secid&hlm=$hlm&msg=$msg");
				exit();
			}
			else
			{
				$error = base64_encode("Data tidak dapat hapus dan silahkan coba kembali");
				header("location: $alamat&aksi=detail&id=$id&secid=$secid&hlm=$hlm&error=$error");
				exit();
			}
		}
	}
	...
	if($aksi=="savereply")
	{
		...
		else
        {
			...
			$pesan_email 	= "
					...
					Jika Anda akan bertanya kembali, silahkan masuk ke menu <strong>Tiket Bantuan</strong>.
					Dengan melakukan login terlebih dahulu. Terima kasih.";
	
			$perintah2	= "insert into $nama_tabel1 (`ticketingbalasid`,`ticketingid`,`isijawaban`,`judul`,`create_date`,`create_userid`,`is_answer`, userid $vfield) 
						values ('$ticketingbalasid','$ticketingid','$isijawaban','$judul','$date','$_SESSION[cms_userid]','$is_answer','$userid' $vval)";
			...
			if($hasil2)
			{
				$qm	= sql("select userfullname, userEmail from tbl_member where userid='$userid'");
	...
	if($aksi=="reply")
	{
		...
		else
        {
			...
			{
				$pelajaranid	= sql_get_var("select pelajaranid from tbl_ticketing_sec where secid='$secid'");
		...
		else
        {
			...
			if($hasil)
			{
				if(!$cekfile)
					...
					$msg = base64_encode("Berhasil mengubah data.");
				header("location: $alamat&aksi=detail&id=$ticketingid&secid=$secid&hlm=$hlm&msg=$msg");
				exit();
			}
			else
			{
				if(!$cekfile)
					...
					$error = base64_encode("Data tidak dapat diubah dan silahkan coba kembali");
				header("location: $alamat&aksi=detail&id=$ticketingid&secid=$secid&hlm=$hlm&error=$error");
				exit();
			}
		}
	}
	...
	if($aksi=="editreply")
	{
		...
		else
        {
			...
			$mainmenu[] = array("Kembali","back","$alamat&aksi=detail&id=$id&secid=$secid");
			mainaction($mainmenu,$param);
